...
func tplTube(conf SelfConf, content string, server string, tube string) string {
	...
	buf.WriteString(dropDownServer(conf, server))
	buf.WriteString(dropDownTube(conf, server, tube))
	...
	buf.WriteString(`</ul>`)
	buf.WriteString(tplSearchTube(conf, server, tube, ""))
